...
Language = Literal["English", "Czech", "Slovakia"]
...
def detect_language_lingua(
    text: str,
) -> Literal["English", "Czech", "Slovakia"]:
    language = detector.detect_language_of(text)

    if language not in (Language.ENGLISH, Language.CZECH, Language.SLOVAK):
        raise ValueError(
            "Unsupported language detected. Only English, Slovak and Czech are supported."
        )

    if language.iso_code_639_1.name == "EN":
        return "English"
    elif language.iso_code_639_1.name == "CZ":
        return "Czech"
    elif language.iso_code_639_1.name == "SK":
        return "Slovakia"
...
class SummarizerWithDetailLevel:
    ...
    async def _summarize_chunk(
        self, chunk: str, bullet_num: int, lang: Language | None
    ) -> tuple[str, list[str]]:
        if lang:
    ...
    async def _refine_merged_summaries(
        self, merged_summaries: str, lang: Language, further_instruction: str
    ) -> str:
        if lang:
    ...
    async def summarize(
        ...
        Args:
            ...
            lang (Language | None): The language of the text.
